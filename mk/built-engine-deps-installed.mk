...
#  setopt -s extglob
#
_REMOVE_ENGINE_RPMS_COMMAND := $(shell sed 's/.*/rm -f \0-[0-9]*.rpm;/g' tested-engine-packages.txt)

%-provision-engine-deps.sh:
	sed 's/REMOVE_RPMS_COMMAND/$(_REMOVE_ENGINE_RPMS_COMMAND)/g' $(PROVISION_ENGINE_DEPS_SCRIPT) > $@

%-engine-deps-installed.qcow2: %-upgrade.qcow2 %-provision-engine-deps.sh
